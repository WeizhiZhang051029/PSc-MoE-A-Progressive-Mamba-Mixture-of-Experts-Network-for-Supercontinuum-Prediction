...
The dataset is generated by numerically solving the generalized nonlinear Schrödinger equation (GNLSE). Each sample is represented as a spectral-evolution map with `200` propagation-distance samples and `251` spectral sampling points.

Please prepare the `.mat` data files and set the corresponding data path in `config.py`.
...
We sincerely thank the open-source community for providing useful implementations of deep-learning models and sequence modeling frameworks.
